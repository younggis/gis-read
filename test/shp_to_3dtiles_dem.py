...
示例:
    ...
    # 贴地白模：从 DEM 采样地面高程，再叠加建筑高度
    python shp_to_3dtiles.py ../data/building.shp ./buildings --dem terrain.asc --height-field HEIGHT
...
说明:
    ...
    5. 可通过 --dem 指定地形 DEM，让建筑底面采样真实地形高程；支持 .asc/.hgt，.tif 需 rasterio。
"""
...
class Feature:
    ...
    # height/base_height 用于 tileset boundingVolume：
    # height = 最高屋顶高程；base_height = 最低底面高程。
    height: float
    base_height: float
    lon_min: float
    lat_min: float
    lon_max: float
    lat_max: float
    # 建筑自身高度。当地形 DEM 逐顶点采样时，屋顶高度 = 顶点地面高程 + building_height。
    building_height: float = 0.0
    # 如果 height 字段被解释为绝对顶高，则这里保存绝对顶高；否则为 None。
    absolute_top_height: Optional[float] = None
    # 与 rings_ll 一一对应的逐顶点底面高程。为 None 时使用 base_height 常量底面。
    vertex_base_heights: Optional[List[List[float]]] = None
...
def is_wgs84_crs(crs: str) -> bool:
    normalized = (crs or "EPSG:4326").upper().replace(" ", "")
    return normalized in {"EPSG:4326", "WGS84", "WGS84_LATLON", "OGC:CRS84", "CRS:84"}


class LonLatToCrsTransformer:
    """EPSG:4326 lon/lat -> DEM CRS.

    DEM 为 EPSG:4326 时不需要 pyproj；DEM 为投影坐标时需要 pyproj。
    """

    def __init__(self, target_crs: str = "EPSG:4326") -> None:
        self.target_crs = target_crs or "EPSG:4326"
        self._transformer = None
        if is_wgs84_crs(self.target_crs):
            return
        try:
            import pyproj  # type: ignore

            self._transformer = pyproj.Transformer.from_crs("EPSG:4326", self.target_crs, always_xy=True)
        except Exception as e:
            raise RuntimeError(
                f"DEM 坐标系为 {self.target_crs}，需要 pyproj 才能从 WGS84 经纬度转换到 DEM 坐标。"
            ) from e

    def transform(self, lon: float, lat: float) -> Vec2:
...
class BaseDemSampler:
    def sample_lonlat(self, lon: float, lat: float) -> Optional[float]:
        raise NotImplementedError


class GridDemSampler(BaseDemSampler):
    """Regular grid DEM sampler with bilinear interpolation."""
    ...
    def __init__(
        ...
        if ncols <= 1 or nrows <= 1:
            raise ValueError("DEM 行列数过小，无法采样。")
        if len(values) != ncols * nrows:
            raise ValueError("DEM 数据量与行列数不一致。")
        self.values = values
        self.ncols = ncols
        self.nrows = nrows
        self.x_first = float(x_first)
        self.y_top = float(y_top)
        self.x_step = float(x_step)
        self.y_step = float(y_step)
        self.nodata = nodata
        self.vertical_offset = float(vertical_offset)
        self.to_dem_crs = LonLatToCrsTransformer(crs)

    def sample_lonlat(self, lon: float, lat: float) -> Optional[float]:
        x, y = self.to_dem_crs.transform(lon, lat)
        return self.sample_xy(x, y)

    def _is_valid(self, v: float) -> bool:
        if not math.isfinite(v):
            return False
        if self.nodata is not None and abs(v - self.nodata) <= 1e-6:
            return False
        return True

    def _value(self, row: int, col: int) -> Optional[float]:
        if row < 0 or row >= self.nrows or col < 0 or col >= self.ncols:
            return None
        v = float(self.values[row * self.ncols + col])
        return v if self._is_valid(v) else None

    def sample_xy(self, x: float, y: float) -> Optional[float]:
        # row/col 以像元中心或 HGT 采样点为基准。
        col_f = (x - self.x_first) / self.x_step
        row_f = (self.y_top - y) / self.y_step
        if col_f < -1e-9 or row_f < -1e-9 or col_f > self.ncols - 1 + 1e-9 or row_f > self.nrows - 1 + 1e-9:
            return None

        col0 = int(math.floor(clamp(col_f, 0.0, self.ncols - 1.0)))
        row0 = int(math.floor(clamp(row_f, 0.0, self.nrows - 1.0)))
        col1 = min(col0 + 1, self.ncols - 1)
        row1 = min(row0 + 1, self.nrows - 1)
        dx = clamp(col_f - col0, 0.0, 1.0)
        dy = clamp(row_f - row0, 0.0, 1.0)

        v00 = self._value(row0, col0)
        v10 = self._value(row0, col1)
        v01 = self._value(row1, col0)
        v11 = self._value(row1, col1)
        vals = [(v00, (1 - dx) * (1 - dy)), (v10, dx * (1 - dy)), (v01, (1 - dx) * dy), (v11, dx * dy)]
        valid = [(v, w) for v, w in vals if v is not None and w > 0.0]
        if valid:
            weight_sum = sum(w for _, w in valid)
            if weight_sum > 0:
                return sum(float(v) * w for v, w in valid) / weight_sum + self.vertical_offset

        # 如果刚好踩在 nodata 交界，回退到最近有效邻点。
        candidates = []
        for rr in range(max(0, row0 - 1), min(self.nrows, row0 + 2)):
            for cc in range(max(0, col0 - 1), min(self.ncols, col0 + 2)):
                v = self._value(rr, cc)
                if v is not None:
                    d2 = (rr - row_f) * (rr - row_f) + (cc - col_f) * (cc - col_f)
                    candidates.append((d2, v))
        if not candidates:
            return None
        candidates.sort(key=lambda t: t[0])
        return float(candidates[0][1]) + self.vertical_offset


class EsriAsciiDemSampler(GridDemSampler):
    """ESRI ASCII Grid DEM: .asc/.grd/.txt

    支持 ncols/nrows/xllcorner|xllcenter/yllcorner|yllcenter/cellsize/NODATA_value。
    """

    def __init__(self, path: Path, crs: str, vertical_offset: float = 0.0, override_nodata: Optional[float] = None) -> None:
        meta: Dict[str, float] = {}
        values: List[float] = []
        with path.open("r", encoding="utf-8", errors="ignore") as f:
            # Header 通常 6 行，但为了兼容大小写和顺序，这里读到关键字段齐全为止。
            while True:
                pos = f.tell()
                line = f.readline()
                if not line:
        ...
        if "xllcenter" in meta:
            x_first = float(meta["xllcenter"])
        else:
            x_first = float(meta["xllcorner"]) + cell * 0.5
        if "yllcenter" in meta:
            y_first = float(meta["yllcenter"])
        else:
            y_first = float(meta["yllcorner"]) + cell * 0.5
        y_top = y_first + (nrows - 1) * cell
        nodata = override_nodata if override_nodata is not None else meta.get("nodata_value", -9999.0)
        super().__init__(values, ncols, nrows, x_first, y_top, cell, cell, nodata, crs, vertical_offset)


class HgtDemSampler(GridDemSampler):
    """SRTM .hgt DEM sampler. Filename must contain NxxEyyy/SxxWyyy."""

    def __init__(self, path: Path, vertical_offset: float = 0.0, override_nodata: Optional[float] = None) -> None:
        import re

        m = re.search(r"([NS])(\d{2})([EW])(\d{3})", path.stem.upper())
        if not m:
            raise ValueError("HGT 文件名需要包含类似 N31E121 或 S07W045 的经纬度编号。")
        lat0 = int(m.group(2)) * (1 if m.group(1) == "N" else -1)
        lon0 = int(m.group(4)) * (1 if m.group(3) == "E" else -1)
        raw = path.read_bytes()
        if len(raw) % 2 != 0:
            raise ValueError("HGT 文件大小不是 2 字节整数的整数倍。")
        count = len(raw) // 2
        n = int(round(math.sqrt(count)))
        if n * n != count or n not in (1201, 3601):
            raise ValueError(f"无法识别 HGT 分辨率：{count} 个采样点。常见为 1201x1201 或 3601x3601。")
        arr = array.array("h")
        arr.frombytes(raw)
        if sys.byteorder == "little":
            arr.byteswap()
        cell = 1.0 / (n - 1)
        nodata = override_nodata if override_nodata is not None else -32768.0
        super().__init__(arr, n, n, lon0, lat0 + 1.0, cell, cell, nodata, "EPSG:4326", vertical_offset)


class RasterioDemSampler(BaseDemSampler):
    """Optional GeoTIFF/COG DEM sampler using rasterio.

    注意：rasterio 不是脚本必需依赖。只有传入 .tif/.tiff DEM 时才会尝试导入。
    """

    def __init__(self, path: Path, fallback_crs: str, vertical_offset: float = 0.0, override_nodata: Optional[float] = None) -> None:
        try:
            import rasterio  # type: ignore
            from rasterio.transform import rowcol  # type: ignore
        except Exception as e:
            raise RuntimeError(
                "读取 GeoTIFF DEM 需要安装 rasterio。若要保持纯 Python，可先把 DEM 转为 ESRI ASCII Grid(.asc) 或 SRTM HGT(.hgt)。"
            ) from e
        self.rasterio = rasterio
        self.rowcol = rowcol
        self.dataset = rasterio.open(path)
        self.band = self.dataset.read(1, masked=False)
        self.nrows, self.ncols = self.band.shape
        self.nodata = override_nodata if override_nodata is not None else self.dataset.nodata
        self.vertical_offset = float(vertical_offset)
        ds_crs = str(self.dataset.crs) if self.dataset.crs else fallback_crs
        self.to_dem_crs = LonLatToCrsTransformer(ds_crs)
    ...
    def sample_lonlat(self, lon: float, lat: float) -> Optional[float]:
        ...
        vals = [
            (float(self.band[row0, col0]), (1 - dx) * (1 - dy)),
            (float(self.band[row0, col1]), dx * (1 - dy)),
            (float(self.band[row1, col0]), (1 - dx) * dy),
            (float(self.band[row1, col1]), dx * dy),
        ]
        valid = [(v, w) for v, w in vals if self._is_valid(v) and w > 0.0]
        if not valid:
            return None
        weight_sum = sum(w for _, w in valid)
        return sum(v * w for v, w in valid) / weight_sum + self.vertical_offset
...
) -> Optional[BaseDemSampler]:
    if not dem_path:
        return None
    path = Path(dem_path).resolve()
    if not path.exists():
        raise FileNotFoundError(f"找不到 DEM 文件：{path}")
    suffix = path.suffix.lower()
    if suffix in (".asc", ".grd", ".txt"):
        return EsriAsciiDemSampler(path, dem_crs, vertical_offset=dem_offset, override_nodata=dem_nodata)
    if suffix == ".hgt":
        return HgtDemSampler(path, vertical_offset=dem_offset, override_nodata=dem_nodata)
    if suffix in (".tif", ".tiff"):
        return RasterioDemSampler(path, dem_crs, vertical_offset=dem_offset, override_nodata=dem_nodata)
    raise ValueError("DEM 格式暂支持：ESRI ASCII Grid(.asc/.grd/.txt)、SRTM HGT(.hgt)、GeoTIFF(.tif/.tiff，需 rasterio)。")


def build_feature_base_heights(
    rings_ll: Sequence[Sequence[Vec2]],
    lon_c: float,
    lat_c: float,
    base_offset: float,
    dem_sampler: Optional[BaseDemSampler],
    sample_mode: str,
    dem_default_height: float,
) -> Tuple[float, float, Optional[List[List[float]]]]:
    """Return min_base, max_base, per-vertex bases or None.

    无 DEM 时：base_offset 是绝对底高。
    有 DEM 时：base = DEM(lon,lat) + base_offset；base_offset 可作为离地偏移/基础高度。
    """
    if dem_sampler is None:
        return float(base_offset), float(base_offset), None

    def sample(lon: float, lat: float) -> float:
        v = dem_sampler.sample_lonlat(lon, lat)
        if v is None:
            return float(dem_default_height)
        return float(v)

    sample_mode = sample_mode.lower()
    vertex_ground: List[List[float]] = [[sample(lon, lat) for lon, lat in ring] for ring in rings_ll]
    flat_ground = [v for ring in vertex_ground for v in ring]
    if not flat_ground:
        g = sample(lon_c, lat_c)
        return g + base_offset, g + base_offset, None

    if sample_mode == "vertices":
        vertex_bases = [[v + base_offset for v in ring] for ring in vertex_ground]
        flat = [v for ring in vertex_bases for v in ring]
        return min(flat), max(flat), vertex_bases

    if sample_mode == "minimum":
        g = min(flat_ground)
    elif sample_mode == "average":
        g = sum(flat_ground) / len(flat_ground)
    else:  # centroid
        g = sample(lon_c, lat_c)
...
def build_tile_mesh(tile: TileBucket, transformer: CoordinateTransformer, rtc_center: Vec3) -> Tuple[List[Vec3], List[Vec3]]:
    ...
    for feature in tile.features:
        for ring_index, ring in enumerate(feature.rings_ll):
            if len(ring) < 3 or abs(signed_area_2d(ring)) <= 1e-20:
                continue

            vertex_bases: Optional[List[float]] = None
            if feature.vertex_base_heights is not None and ring_index < len(feature.vertex_base_heights):
                vertex_bases = list(feature.vertex_base_heights[ring_index])
                if len(vertex_bases) != len(ring):
                    vertex_bases = None

            # 保持 ring 与逐顶点高程同步翻转。
            if signed_area_2d(ring) < 0:
                ring_ccw = list(reversed(ring))
                if vertex_bases is not None:
                    vertex_bases = list(reversed(vertex_bases))
            else:
                ring_ccw = list(ring)
            ...
            if vertex_bases is None:
                base_heights = [feature.base_height for _ in ring_ccw]
            else:
                base_heights = vertex_bases

            if feature.absolute_top_height is None:
                top_heights = [bh + feature.building_height for bh in base_heights]
            else:
                top_heights = [feature.absolute_top_height for _ in base_heights]

            # 避免顶面低于底面导致模型反向或消失。
            top_heights = [max(th, bh + 0.01) for th, bh in zip(top_heights, base_heights)]

            bottom = [local_ecef(lon, lat, h) for (lon, lat), h in zip(ring_ccw, base_heights)]
            top = [local_ecef(lon, lat, h) for (lon, lat), h in zip(ring_ccw, top_heights)]
...
def convert(args: argparse.Namespace) -> None:
    ...
    dem_sampler = load_dem_sampler(args.dem, args.dem_crs, args.dem_offset, args.dem_nodata)
    # 指定 DEM 后，建筑高度字段默认按“相对地面高度”解释，更符合贴地白模习惯；
    # 如确实是绝对屋顶高程，可加 --height-absolute。
    height_is_relative = args.height_is_relative or (dem_sampler is not None and not args.height_absolute)
    ...
    for idx, shp_rec in enumerate(read_shp_polygons(input_shp, args.limit)):
        ...
        raw_height = get_height(attrs, height_field, args.default_height)
        base_offset = get_height(attrs, base_height_field, args.base_height) if base_height_field else float(args.base_height)
        if not math.isfinite(raw_height):
            raw_height = float(args.default_height)
        if not math.isfinite(base_offset):
            base_offset = 0.0

        base_h, max_base_h, vertex_base_heights = build_feature_base_heights(
            exterior_ll_rings,
            lon_c,
            lat_c,
            float(base_offset),
            dem_sampler,
            args.dem_sample,
            float(args.dem_default_height),
        )

        absolute_top_height: Optional[float] = None
        if height_is_relative:
            building_h = max(float(raw_height), 0.0)
        else:
            absolute_top_height = float(raw_height)
            building_h = absolute_top_height - base_h

        if building_h <= 0.01:
            building_h = max(float(args.default_height), 1.0)
            absolute_top_height = None
        if args.min_height is not None and building_h < args.min_height:
            building_h = args.min_height
            absolute_top_height = None if height_is_relative else base_h + building_h
        if args.max_height is not None and building_h > args.max_height:
            building_h = args.max_height
            absolute_top_height = None if height_is_relative else base_h + building_h

        top_h = (max_base_h + building_h) if absolute_top_height is None else max(absolute_top_height, max_base_h + 0.01)
        ...
        feature = Feature(
            ...
            absolute_top_height=absolute_top_height,
            vertex_base_heights=vertex_base_heights,
        )
    ...
    root_east = -180.0
    root_north = -90.0
    root_min_h = float("inf")
    root_max_h = float("-inf")
    ...
    if args.dem:
        print(f"  dem              : {Path(args.dem).resolve()}")
        print(f"  dem sample       : {args.dem_sample}")
        print(f"  height mode      : {'relative' if height_is_relative else 'absolute'}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--base-height", type=float, default=0.0, help="默认底部高度，单位米，默认 0。指定 --dem 时作为离地偏移量叠加到 DEM 高程。")
    p.add_argument("--dem", default=None, help="可选 DEM 文件，用于给建筑底面采样地形高程。支持 .asc/.hgt；.tif/.tiff 需 rasterio。")
    p.add_argument("--dem-crs", default="EPSG:4326", help="DEM 坐标系，默认 EPSG:4326。GeoTIFF 若自身有 CRS 会优先使用自身 CRS。")
    p.add_argument("--dem-offset", type=float, default=0.0, help="DEM 高程整体偏移，单位米；可用于修正垂直基准差。默认 0。")
    p.add_argument("--dem-nodata", type=float, default=None, help="手动指定 DEM nodata 值；不指定则读取文件默认值。")
    p.add_argument("--dem-default-height", type=float, default=0.0, help="DEM 超范围或 nodata 时使用的地面高程，默认 0。")
    p.add_argument(
        "--dem-sample",
        choices=("vertices", "centroid", "minimum", "average"),
        default="vertices",
        help="DEM 采样方式：vertices=每个建筑顶点独立采样，贴地最好；centroid/minimum/average=整栋建筑使用一个底高。默认 vertices。",
    )
    p.add_argument(
        "--height-is-relative",
        action="store_true",
        help="把 height 字段解释为相对高度；topHeight = baseHeight + height。默认无 DEM 时 height 字段视为绝对顶高。",
    )
    p.add_argument(
        "--height-absolute",
        action="store_true",
        help="指定 --dem 时仍把 height 字段解释为绝对屋顶高程；默认指定 DEM 后 height 字段按相对建筑高度处理。",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.height_is_relative and args.height_absolute:
        parser.error("--height-is-relative 和 --height-absolute 不能同时使用。")
